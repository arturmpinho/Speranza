...
|After editing a comment, the comment shows with the new date/time, altering the order of conversations.	|When e.g. two users engage in a conversation it should be intercalated. Nonetheless, when one of the users edit a comment/review, this will acquire the new time/date info, altering the order of the conversations.	| N/A   |**Not fixed**. In the future I want to order the comments by the 'posted_on' field and create an additional field - last_edit - grabing the editing date/time. This way, the conversations will always be intercalated and won't create any confusion to the user.	|
